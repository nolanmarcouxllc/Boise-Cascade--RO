...
import { getSessionContext, getViewerContext, type SessionContext } from "@/lib/auth";
...
/**
 * Guard for READ-ONLY endpoints that are safe to expose in public demo mode
 * (the fleet map, finding details, the comparison). Rate-limits by IP, then
 * resolves either the real signed-in org or — for an anonymous visitor in public
 * demo mode — the read-only demo org. Never use this for a write.
 */
export async function requirePublicOrg(request?: Request): Promise<Guard> {
  if (request) {
    const rl = checkRateLimit(`api:${clientIp(request)}`, { windowMs: 60_000, max: 120, hardBlock: 400 });
    if (!rl.ok) {
      return {
